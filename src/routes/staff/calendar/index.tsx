...
function StaffCalendarPage() {
  ...
  // Count deals with closing dates
  const dealsWithClosingDate = dealsQuery.data?.deals.filter(
    (deal) => deal.closingDate !== null
  ).length || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-primary-50/50 to-white rounded-xl p-5 md:p-6 shadow-md border-2 border-primary-100 animate-in fade-in slide-in-from-top-4 duration-300">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Deal Calendar</h1>
            <p className="text-gray-600 text-base md:text-lg">
              View all deals organized by their closing dates
            </p>
          </div>
          <div className="flex items-center gap-3 bg-white/80 backdrop-blur-sm px-6 py-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900">
                {dealsWithClosingDate}
              </div>
              <div className="text-xs text-gray-600 font-medium">Scheduled Deals</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {dealsQuery.isLoading ? (
        ...
        <CalendarView deals={dealsQuery.data?.deals || []} />
      )}
    </div>
  );
}
