...
export function AddressAutocomplete({
  ...
  const handleSelectSuggestion = async (placeId: string, description: string) => {
    ...
    try {
      const response = await trpc.getPlaceDetails.query({ placeId });
      onSelectAddress({
          ...
          <div className="max-h-80 overflow-y-auto overscroll-contain">
            {suggestions.map((suggestion, index) => (
              <button
                ...
                className={`w-full text-left px-4 py-4 transition-all flex items-start gap-3 min-h-[60px] touch-manipulation ${
                  index === selectedIndex
